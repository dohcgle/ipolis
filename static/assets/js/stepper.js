...
function validateStepThree() {
  var validation = true;
  if (
    $('.step:nth-child(3) input[type="text"]')
      .val()
      .indexOf("materialize") === -1
  )
    validation = false;
...
if (validation) {
  $.validator.setDefaults({
    ...
    success: function (element) {
      if (
        !$(element).closest("li").find("label.invalid:not(:empty)").length
      ) {
...
$.fn.openStep = function (step, callback) {
  ...
  var active = this.find(".step.active");
  var next;
  var prev_active = (next = $(this.children(".step:visible")).index(
    $(active)
  ));
